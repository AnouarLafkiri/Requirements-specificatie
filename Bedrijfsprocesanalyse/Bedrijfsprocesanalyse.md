...
| 3 | Dashboard vernieuwen | BI-dashboards worden automatisch bijgewerkt op basis van de meest recente data in het datawarehouse. | Systeem (automatisch) | Apache Superset | Automatisch |
...
De overige gaps (beveiliging, adoptie, schaalbaarheid) zijn oplosbaar binnen de bestaande kaders van Apache Superset als open source BI-tool, mits de implementatie gefaseerd wordt uitgevoerd en eindgebruikers vroeg worden betrokken. De verwachte tijdsbesparing van ~95% per rapportagecyclus rechtvaardigt de investering zelfs bij een minimaal budget.
